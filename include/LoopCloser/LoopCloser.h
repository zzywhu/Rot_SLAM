...
class LOOP_CLOSER_API LoopCloser {
 public:
  struct LOOP_CLOSER_API Config {
    ...
    Config() {
      ...
      _minIOUThre =
          0.;  // 0.4; // min boundingbox iou for candidate registration edge
      _minIOUThreGlobalReg = 0.5;
      _neighborSearchDist =
          6.0;                // max distance for candidate registration edge
      _minSubmapIdDiff = 20;  // 8; //min submap id difference between two
                              // submaps for a putable registration edge
      ...
      _isTransferCorrectRegTranOn =
          true;  // enable the registration tranformation transfer (only do
                 // global reg. once for each query submap
      _maxUsedRegEdgePerOptimization = 3;
      _cloudPCANeighRadius = 2.0;  // 0.6;    // pca neighborhood searching
                                   // radius(m) for target point cloud
      _globalRegMinInlierCount =
          7;  // min inlier correspondence for a successful feature based
              // registration (for teaser or ransac)
      _wrongEdgeTranThre = 5.0;  // translation threshold for judging if a edge
                                 // is wrong or not");
      _wrongEdgeRotThreDeg =
          25.0;  // rotation threshold for judging if a edge is wrong or not");
      _poseGraphOptimizationMethod =
          "gtsam";  //"gtsam";//"ceres"; // use which library to do pgo (select
                    //from g2o, ceres and gtsam
      _coolingSubmapNum =
          10;  // waiting for several submaps (without loop closure detection)
               // after applying a successful pgo
      _isRobustKernelOn = false;  // turn on the robust kernel function in pgo
      _isEqualWeightOn = false;   // using equal weight for the information
                                  // matrix in pose graph optimization
      _maxIterInterSubmap = 100;  // max iteration number for inter submap pgo
      _isDiagonalInformationMatrixOn =
          false;             // use diagonal information matrix in pgo or not
      ...
      _tInterSubmapLimit =
          2.0;  // the submap node's limit of translation variation, unit:m
      _RInterSubmapLimit = 0.5;  // 0.05; //the submap node's limit of rotation
                                 // variation, unit quaternion
 ...
 public:
  LoopCloser()
    ...
    _pgOptimizer.set_diagonal_information_matrix(
        _config._isDiagonalInformationMatrixOn);
  ...
  void inputFrameData(int frameId, const double& timeStamp, const M3D& rot,
                      const V3D& pos, PointCloudXYZI::Ptr localCloud,
  ...
  int getFrameStripID(const int& frameID) {
    if (_frameIDMap.find(frameID) == _frameIDMap.end()) return -1;
  ...
  std::unordered_map<VOXEL_LOC, OctoTree*>& getVoxelSurfMap() {
    return _voxelSurfMap;
  }
