...
#define RESULT_FILE_DIR(name) \
  (std::string(std::string(ROOT_DIR) + "result/" + name))
...
struct MatchedInfo {
  MatchedInfo(const V3D &pl, const Eigen::Vector4d &pabcd, const V3D &center,
              const V3D &norm, const double &coeffs) {
    _pl = pl;
    _center = center;
    _norm = norm;
    _pabcd = pabcd;
...
void COMMON_API calcBodyCov(PointWithCov &pv, const float range_inc,
                            const float degree_inc);

void COMMON_API calcPointNeighCov(std::vector<PointWithCov> &pvList,
...
Eigen::Matrix<T, 3, 3> AngleAxisToRotationMatrix(
  ...
  T angle = rvec.norm();
  if (angle == T(0)) return Eigen::Matrix<T, 3, 3>::Identity();
...
template <class T>
void calc(T matrix[4][5], Eigen::Vector3d &solution)  //计算行列式
{
...
double COMMON_API calVelocity(Matrix4ds &Trans, int mean_frame_num = 20,
