...
#endif
using namespace std;
// This class is used to calculate the position and attitude of the lidar to the
// IMU
class ZG_TRANSFER_API ZGAxisTransfer {
 ...
 public:
  ...
  bool ReadEquipmentParams(const char *path) {
    ...
    fscanf(fp, "%lf %lf %lf %lf %lf %lf", &benchmarkA, &shift_Z_A, &D, &X, &Y,
           &Z);
  ...
  bool readZGExtrinsic(const std::string &paramPath) {
    ...
    fscanf(fp, "%lf %lf %lf %lf %lf %lf", &motoraxis(0), &motoraxis(1),
           &motoraxis(2), &trans(0), &trans(1), &trans(2));

    _RLI_calib.matrix() << 0, 0.939693, 0.34202, 0.122378, -1, 0, 0, 0.033, 0,
        -0.34202, 0.939693, 0.12356, 0, 0, 0, 1;
    Eigen::Matrix3d rotMatrix;
    Eigen::Vector3d vectorBefore(0, 1, 0);
    rotMatrix = Eigen::Quaterniond::FromTwoVectors(vectorBefore, motoraxis)
                    .toRotationMatrix();
  ...
  // Set reference angle
  void SetBenchmarkAngle(const double angle) { m_benchmark_angle = angle; }
  ...
  // Calculate posture
  void CalculatePose(const double &angle, Eigen::Matrix3d &R,
                     Eigen::Vector3d &T) {
    // 1 calculate error
    double error[6] = {0};
    ...
    Eigen::Vector3d lidar_coord =
        RodriguesRotation(angle_gap_rad, m_rotation_line,
                          m_lidar_2_rotation_center) +
        m_rotation_center;
    Eigen::Vector3d lidar_X =
        RodriguesRotation(angle_gap_rad, m_rotation_line, m_lidar_X);
    Eigen::Vector3d lidar_Y = m_lidar_Y;
    Eigen::Vector3d lidar_Z =
        RodriguesRotation(angle_gap_rad, m_rotation_line, m_lidar_Z);
    lidar_X.normalize();
    lidar_Y.normalize();
    lidar_Z.normalize();
  ...
  void getLid2IMUTrans(const double &motorAngle, Eigen::Matrix3d &Ril,
                       Eigen::Vector3d &til) {
  ...
  void getLid2IMUTransRefine(const double &motorAngle, Eigen::Matrix3d &Ril,
                             Eigen::Vector3d &til) {
    Eigen::Affine3d Til =
        _RLI_calib * _fixPart_new.inverse() *
        Eigen::AngleAxisd(-motorAngle, Eigen::Vector3d::UnitY()) * _fixPart_new;

    Ril = Til.linear();
    til = Til.translation();
  }

  void getLid2IMUTransRefineforSIM(const double &motorAngle,
                                   Eigen::Matrix3d &Ril, Eigen::Vector3d &til) {
    _RLI_calib.matrix() << 0.8658698, 0.0000000, -0.5002694, -0.089, 0.0000000,
        1.0000000, 0.0000000, -0.000, 0.5002694, 0.0000000, 0.8658698, 0.288, 0,
        0, 0, 1;
    Eigen::Affine3d Til =
        _RLI_calib * _fixPart_new.inverse() *
        Eigen::AngleAxisd(motorAngle, Eigen::Vector3d::UnitX()) * _fixPart_new;
    ...
    //           << Ril << std::endl;
    // std::cout << "til: \n"
    //           << til.transpose() << std::endl;
  }

 private:
  // Rodri's rotation formula
  Eigen::Vector3d RodriguesRotation(double angle, Eigen::Vector3d rotation_vec,
                                    Eigen::Vector3d &vec) {
    return cos(angle) * vec +
           (1 - cos(angle)) * (vec.dot(rotation_vec)) * rotation_vec +
           sin(angle) * rotation_vec.cross(vec);
  }

  // Angle to radian
  double GetRad(double angle) { return angle * 3.14159265358 / 180; }

  // Calculate error
  double CalcError(Eigen::VectorXd param, double A) {
    double E = param(0) * pow(A, 7) + param(1) * pow(A, 6) +
               param(2) * pow(A, 5) + param(3) * pow(A, 4) +
               param(4) * pow(A, 3) + param(5) * pow(A, 2) +
               param(6) * pow(A, 1) + param(7) +
               param(8) * sin(param(9) * A + param(10));
    return E;
  }
 ...
 private:
  ...
  Eigen::Vector3d
      m_lidar_2_rotation_center;  // Lidar origin to center of rotation vector
...
class ZG_TRANSFER_API Interpolation {
 ...
 public:
  ...
  // 2. angle->[0,360]
  bool CalculatePose(const double angle, Eigen::Matrix3d &R,
                     Eigen::Vector3d &T) {
...
*/
 ...
 private:
  // Rodri's rotation formula
  inline Eigen::Vector3d RodriguesRotation(double angle,
  ...
  // Angle to radian
  inline double GetRad(double angle) { return angle * 3.14159265358 / 180; }
