...
void motorCallback(const scout_gazebo::LidarAngle::ConstPtr &msgIn) {
    ...
    //std::cout << std::fixed << std::setprecision(6) << "Motor time: " << timestamp << std::endl;
#if OUTPUT_LOG
    ...
    if (timestamp >= _sys->_lastMotorTimestamp) {
        ...
        //std::cout << std::fixed << std::setprecision(6) << "Motor angle:" << msgIn->motor_angle << " at " << timestamp << std::endl;
    } else {
