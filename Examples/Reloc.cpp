...
void publishPreMap(const ros::Publisher &pubLaserCloudPreMap) {
    ...
    //std::cout << "publish premap size: " << downsampled->points.size() << std::endl;
}
...
int main(int argc, char **argv) {
    ...
    _sys->bev_manager = std::make_shared<BEVFeatureManager>(_nh, _sys->projectormap);
    ...
    ros::Publisher pubPreMap = _nh.advertise<sensor_msgs::PointCloud2>(_preMapTopic, 10);
    ros::Publisher pubOdom2Map = _nh.advertise<nav_msgs::Odometry>(_odom2mapTopic, 10);
    ros::Publisher pubbevmatch = _nh.advertise<sensor_msgs::Image>(_bevmatchTopic, 1, true);
    ...
    std::cout << "PreMap Loaded, point size: " << _sys->_premapPtr->points.size() << std::endl;
    //发布预先加载的地图
    sensor_msgs::PointCloud2 laserCloudmsg;
    ...
    cv::imwrite(std::string(ROOT_DIR) + "BEV/database/Map_BEV.png", local_map_frame.img_dense);

    // 对premap进行降采样 - 只在初始化时执行一次
    pcl::PointCloud<pcl::PointXYZI>::Ptr premap_downsampled(new pcl::PointCloud<pcl::PointXYZI>);
    pcl::VoxelGrid<pcl::PointXYZI> premap_voxel_grid;
    premap_voxel_grid.setInputCloud(_sys->_premapPtr);
    premap_voxel_grid.setLeafSize(_premapdownsampleLeaf, _premapdownsampleLeaf, _premapdownsampleLeaf);
    premap_voxel_grid.filter(*premap_downsampled);
    std::cout << "PreMap downsampled: " << _sys->_premapPtr->points.size()
              << " -> " << premap_downsampled->points.size() << " points" << std::endl;

    pcl::PointCloud<pcl::PointXYZI>::Ptr bev_downsampled(new pcl::PointCloud<pcl::PointXYZI>);
        ...
        if (!_sys->_cloudBuffer.empty()) {
            ...
            if (!is_fully_localized) {
                if (!_sys->_mapBuffer.empty() && _sys->_frameId % 50 == 0) {
                    std::cout << "Processing point clouds for localization..." << std::endl;
                    ...
                    std::cout << "Merged cloud size: " << merged_cloud->points.size() << std::endl;

                    if (merged_cloud->points.size() >= 1000) {
                        // 2. 转换为BEV格式的点云
                        pcl::PointCloud<pcl::PointXYZI>::Ptr bev_cloud(new pcl::PointCloud<pcl::PointXYZI>);
                        ...
                        // 遍历不同旋转角度进行匹配
                        for (double angle_deg = -max_angle_deg; angle_deg <= max_angle_deg; angle_deg += angle_step_deg) {
                            std::cout << "\n--- Trying localization with rotation: " << angle_deg << " degrees ---" << std::endl;
                            ...
                            pcl::PointCloud<pcl::PointXYZI>::Ptr rotated_bev_cloud(new pcl::PointCloud<pcl::PointXYZI>);
                            pcl::transformPointCloud(*bev_cloud, *rotated_bev_cloud, rotation_transform);
                            ...
                            std::cout << "Starting XFeat matching with system detector..." << std::endl;
                            cv::Mat mkpts_0, mkpts_1;
                            _sys->_XFDetector.match_xfeat(current_frame.img_dense,
                                                          local_map_frame.img_dense,
                                                          mkpts_0,
                                                          mkpts_1);
                            std::cout << "XFeat found " << mkpts_0.rows << " initial matches" << std::endl;

                            // 5. 如果有足够的匹配点，进行RANSAC粗差剔除
                            if (mkpts_0.rows > 10) {
                                std::cout << "Processing " << mkpts_0.rows << " XFeat matches with RANSAC..." << std::endl;
                                ...
                                cv::Mat mask;
                                cv::Mat H = cv::findHomography(pts_current, pts_map,
                                                               cv::RANSAC, 20.0, mask,
                                                               100, 0.99);
                                ...
                                if (!H.empty()) {
                                    ...
                                    std::cout << "RANSAC result: " << num_inliers << "/" << pts_current.size() << " inliers" << std::endl;
                                    ...
                                    if (num_inliers > 8) { // 需要足够的内点才进行可视化
                                        ...
                                        if (!keypoints_current.empty() && !keypoints_map.empty() && !good_matches.empty()) {
                                            cv::drawMatches(current_frame.img_dense, keypoints_current,
                                            ...
                                            // 在图像上添加文本信息
                                            std::string info_text = "Inliers: " + std::to_string(num_inliers) + "/" + std::to_string(mkpts_0.rows);
                                            cv::putText(img_matches, info_text, cv::Point(10, 30),
                                                        cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 255, 0), 2);

                                            std::cout << "Created visualization with " << good_matches.size()
                                                      << " inlier matches" << std::endl;
                                        }
                                    } else {
                                        std::cout << "Not enough inliers for visualization: " << num_inliers << std::endl;
                                        ...
                                        std::string warning_text = "Poor matches: " + std::to_string(num_inliers) + "/" + std::to_string(mkpts_0.rows);
                                    ...
                                    if (!img_matches.empty()) {
                                        sensor_msgs::ImagePtr match_img_msg = cv_bridge::CvImage(current_header, "bgr8", img_matches).toImageMsg();
                                        pubbevmatch.publish(match_img_msg);
                                        std::cout << "Published RANSAC-filtered match image with " << num_inliers << " inliers" << std::endl;
                                    }

                                    // //打印2d点对
                                    // for (int i = 0; i < pts_current.size(); i++) {
                                    //     std::cout << "Match " << i << ": Current(" << pts_current[i].x << ", " << pts_current[i].y
                                    //               << ") <-> Map(" << pts_map[i].x << ", " << pts_map[i].y << ")" << std::endl;
                                    // }
                                    ...
                                    if (num_inliers > 8) {
                                        ...
                                        //     0.99        // 置信度
                                        // );
                                        // 仅使用上一阶段 Homography RANSAC 的内点再次估计 2D 刚性初值
                                        std::vector<cv::Point2f> inlier_curr,
                                            inlier_map;
                                        inlier_curr.reserve(num_inliers);
                                        inlier_map.reserve(num_inliers);
                                        for (int i = 0; i < mkpts_0.rows; ++i) {
                                            if (mask.at<uchar>(i, 0)) {
                                                inlier_curr.push_back(_sys->projectorcurrent->backProjectBEVPixelToXY(mkpts_0.at<cv::Point2f>(i, 0)));
                                                inlier_map.push_back(_sys->projectormap->backProjectBEVPixelToXY(mkpts_1.at<cv::Point2f>(i, 0)));
                                            }
                                        }
                                        //打印所有内点点对
                                        for (int i = 0; i < inlier_curr.size(); i++) {
                                            std::cout << "Inlier " << i << ": Current(" << inlier_curr[i].x << ", " << inlier_curr[i].y
                                                      << ") <-> Map(" << inlier_map[i].x << ", " << inlier_map[i].y << ")" << std::endl;
                                        }
                                        if (inlier_curr.size() < 3) {
                                            std::cout << "Inlier pairs < 3, skip rigid init." << std::endl;
                                            continue;
                                        }
                                        cv::Mat inliers_rigid;
                                        cv::Mat A = cv::estimateAffinePartial2D(
                                            inlier_curr, inlier_map, inliers_rigid,
                                            cv::RANSAC,
                                            1.0, // 像素阈值(可调 1~3)
                                            2000,
                                            0.99);
                                        if (A.empty()) {
                                            std::cout << "Failed to estimate 2D rigid transform with RANSAC" << std::endl;
                                            continue;
                                        ...
                                        std::cout << "Estimated scale from 2D rigid: " << s << std::endl;
                                        float yaw = 0.f;
                                        if (s > 1e-8) {
                                            double r00 = a00 / s;
                                            double r10 = a10 / s;
                                        ...
                                        std::cout << "Initial estimate from rigid 2D (inliers only) - dx: " << dx
                                                  << ", dy: " << dy << ", yaw: " << yaw << std::endl;

                                        // 8. 构建初始变换矩阵 (这是从旋转后的帧到地图的变换)
                                        Eigen::Matrix4f initial_transform_from_rotated = Eigen::Matrix4f::Identity();
                                        ...
                                        Eigen::Matrix4f initial_transform = initial_transform_from_rotated * rotation_transform;
                                        ...
                                        bev_voxel_grid.setLeafSize(_premapdownsampleLeaf, _premapdownsampleLeaf, _premapdownsampleLeaf);
                                        bev_voxel_grid.filter(*bev_downsampled);
                                        //bev_cloud = bev_downsampled;
                                        std::cout << "BEV cloud downsampled: " << bev_cloud->points.size() << " points" << std::endl;
                                        // ... GICP配准代码保持不变 ...
                                        //pcl::PointCloud<pcl::PointXYZI>::Ptr bev_cloud_aligned(new pcl::PointCloud<pcl::PointXYZI>);
                                        //pcl::transformPointCloud(*bev_downsampled, *bev_cloud_aligned, initial_transform);
                                        //pcl::io::savePCDFileBinary(std::string(ROOT_DIR) + "BEV/query/current_bev.pcd", *bev_cloud_aligned);
                                        ...
                                        gicp.align(*aligned_cloud, initial_transform); // 使用修正后的初值
                                        auto end_time = std::chrono::high_resolution_clock::now();
                                        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time);

                                        std::cout << "GICP alignment completed in " << duration.count() << " ms" << std::endl;

                                        if (gicp.hasConverged()) {
                                            double fitness_score = gicp.getFitnessScore();
                                            std::cout << "GICP converged with fitness score: " << fitness_score << std::endl;

                                            // GICP的结果已经是最终的 map <- odom 变换，无需再乘 rotation_transform
                                            ...
                                            if (fitness_score < localization_threshold) {
                                                ...
                                                std::cout << "Fully localized. Stop further GICP computation." << std::endl;
                                            } else {
                                                std::cout << "Partially localized. Continue refining..." << std::endl;
                                            }
                                        }

                                    } else {
                                        std::cout << "Not enough RANSAC inliers for localization: " << num_inliers << std::endl;
                                    }
                                } else {
                                    std::cout << "Failed to estimate Homography with XFeat matches" << std::endl;
                                }
                            } else {
                                std::cout << "Not enough XFeat matches for RANSAC: " << mkpts_0.rows << std::endl;
                            }
                            // 如果已经完全定位，则跳出角度搜索循环
                            if (is_fully_localized) {
                                break;
                        ...
                        std::cout << "Not enough points for localization, continuing..." << std::endl;
                    }
                }
                std::cout << "Localization status: "
                          << (is_fully_localized ? "Fully localized" : (is_localized ? "Partially localized" : "Not localized"))
                          << std::endl;
                ...
                if (is_localized) {
                    ...
                    tf_broadcaster.sendTransform(tf::StampedTransform(
                        transform, ros::Time().fromSec(_sys->_lidarBegTime), "map", "odom"));
                } else {
                    tf::Transform identity_transform;
                    identity_transform.setIdentity();
                    tf_broadcaster.sendTransform(tf::StampedTransform(
                        identity_transform, ros::Time().fromSec(_sys->_lidarBegTime), "map", "odom"));
                }
            } else {
                // 已彻底定位：不再计算，仅发布最终结果
                std::cout << "Already fully localized. Publishing final odometry only." << std::endl;
                final_odom.header.stamp = ros::Time().fromSec(_sys->_lidarBegTime);
                pubOdom2Map.publish(final_odom);
