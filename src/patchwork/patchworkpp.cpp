...
void PatchWorkpp::estimateGround(Eigen::MatrixXf cloud_in) {
    ...
    // shift all z by -10.0 before detection
    const float z_shift = 10.0f;
    for (int i = 0; i < cloud_in.rows(); ++i) {
        cloud_in(i, 2) -= z_shift;
    }
    ...
    // add back +10.0 to z of the results
    for (auto &p : cloud_ground_) p.z += z_shift;
    for (auto &p : cloud_nonground_) p.z += z_shift;
