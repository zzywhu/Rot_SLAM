...
void XFDetector::detectAndCompute(
    torch::Tensor& x, std::unordered_map<std::string, torch::Tensor>& result) {
  torch::Device device(device_type);
  x = x.to(device);
  ...
  auto mkpts_x =
      mkpts.index({torch::indexing::Ellipsis, 0})
          .gather(-1, idxs)
          .index({torch::indexing::Slice(), torch::indexing::Slice(0, top_k)});
  auto mkpts_y =
      mkpts.index({torch::indexing::Ellipsis, 1})
          .gather(-1, idxs)
          .index({torch::indexing::Slice(), torch::indexing::Slice(0, top_k)});
...
void XFDetector::match(torch::Tensor& feats1, torch::Tensor& feats2,
...
torch::Tensor XFDetector::getKptsHeatmap(torch::Tensor& kpts,
                                         float softmax_temp) {
...
torch::Tensor XFDetector::NMS(torch::Tensor& x, float threshold,
                              int kernel_size) {
  int B = x.size(0);
  int H = x.size(2);
  int W = x.size(3);
